...
<head>
    ...
    <title>SENA Digiturno — Panel Asesor</title>
    ...
    <style>
        ...
        @keyframes pulseg {
            0%   { box-shadow: 0 0 0 0 rgba(57,169,0,0.5); }
            70%  { box-shadow: 0 0 0 14px rgba(57,169,0,0); }
            100% { box-shadow: 0 0 0 0 rgba(57,169,0,0); }
        }
        .queue-item { transition: all 0.3s ease; }
        .queue-item:hover { border-color: #39a900; transform: translateX(4px); }
        .btn-primary   { background: #39a900; transition: all 0.2s; }
        .btn-primary:hover   { background: #2d8500; transform: translateY(-1px); }
        ...
        .btn-warning   { background: #b45309; transition: all 0.2s; }
        .btn-warning:hover   { background: #92400e; }
        .btn-danger    { background: #991b1b; transition: all 0.2s; }
        .btn-danger:hover    { background: #7f1d1d; }
        .btn-info      { background: #1d4ed8; transition: all 0.2s; }
    ...
    <header class="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <div class="w-10 h-10 bg-white rounded-xl p-1.5 flex items-center justify-center">
                <img src="{{ asset('images/logosena.png') }}" alt="SENA">
            </div>
            <div>
                <span class="text-white font-black text-lg uppercase tracking-tight">SENA <span class="text-[#39a900]">Digiturno</span></span>
        ...
        <div class="flex items-center gap-4">
            ...
            <div id="estado-badge" class="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider
                {{ $asesor->ase_estado === 'disponible' ? 'bg-[#39a900]/20 text-[#39a900] border border-[#39a900]/30' :
                   ($asesor->ase_estado === 'en_espera'  ? 'bg-yellow-600/20 text-yellow-400 border border-yellow-600/30' :
                                                           'bg-blue-600/20 text-blue-400 border border-blue-600/30') }}">
                <span class="w-2 h-2 rounded-full inline-block
                    {{ $asesor->ase_estado === 'disponible' ? 'bg-[#39a900] pulse-green' :
                       ($asesor->ase_estado === 'en_espera'  ? 'bg-yellow-400' : 'bg-blue-400') }}"></span>
        ...
        <div class="lg:col-span-1 flex flex-col gap-5">
            ...
            <div class="glass rounded-2xl p-6">
                ...
                <div class="flex items-center gap-4 mb-5">
                    <div class="w-14 h-14 bg-[#39a900]/10 border border-[#39a900]/30 rounded-2xl flex items-center justify-center text-2xl font-black text-[#39a900]">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-black text-[#39a900]">
            ...
            <div id="card-turno-actual" class="{{ $asesor->ase_estado === 'ocupado' ? '' : 'hidden' }} glass rounded-2xl p-6 border border-blue-500/30">
                ...
                <div class="flex gap-2 mt-2">
                    ...
                    <button
                        ...
                        class="btn-danger flex-1 text-white font-extrabold py-2.5 rounded-xl uppercase tracking-widest text-xs"
                    >
                        ✓ Finalizar
                    </button>
                </div>
            ...
            <div class="glass rounded-2xl p-6 space-y-3">
                ...
                <button
                    ...
                    class="btn-primary w-full text-white font-extrabold py-4 rounded-xl uppercase tracking-widest text-sm flex items-center justify-center gap-2 {{ $asesor->ase_estado !== 'disponible' ? 'opacity-40 cursor-not-allowed' : 'pulse-green' }}"
            ...
            <div class="glass rounded-2xl p-5 flex items-center justify-between">
                <div>
                    ...
                    <p id="contador-atendidos" class="text-4xl font-black text-[#39a900] mt-1">{{ count($historial) }}</p>
                </div>
                <div class="w-14 h-14 bg-[#39a900]/10 border border-[#39a900]/30 rounded-2xl flex items-center justify-center">
                    <svg class="w-7 h-7 text-[#39a900]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                </div>
            </div>
        </div>
            ...
            <div class="glass rounded-2xl p-6">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-2 h-2 bg-[#39a900] rounded-full animate-pulse"></div>
                        <span class="text-[#39a900] text-xs font-black uppercase tracking-wider">En vivo</span>
                        <span class="glass text-white text-xs font-black px-3 py-1 rounded-full ml-2">
                    ...
                    @forelse($historial as $item)
                        <div class="flex items-center justify-between glass rounded-xl px-5 py-3 border border-white/5">
                            <div class="flex items-center gap-3">
                                <span class="text-[#39a900] font-black text-lg">{{ $item['codigo'] }}</span>
                            </div>
    ...
    <div id="modal-persona" class="fixed inset-0 z-50 hidden items-center justify-center p-4" style="background:rgba(0,0,0,0.8);backdrop-filter:blur(6px);">
        <div class="glass rounded-3xl p-8 w-full max-w-md border border-white/10 shadow-2xl">
            ...
            <form id="forma-persona" class="space-y-4 hidden" onsubmit="guardarPersona(event)">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="pers_tipodoc" id="f-tipodoc" required
                            class="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-white font-semibold text-sm focus:border-[#39a900] outline-none transition-all">
                    ...
                    <div class="col-span-2">
                        ...
                        <input id="f-nombres" name="pers_nombres" type="text" required placeholder="Nombres completos"
                            class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-semibold text-sm focus:border-[#39a900] outline-none transition-all">
                    </div>
                ...
                <div id="modal-success" class="hidden bg-green-900/30 border border-green-500/30 text-[#39a900] rounded-xl px-4 py-3 text-sm font-semibold">✓ Datos guardados correctamente</div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="btn-primary flex-1 text-white font-extrabold py-3 rounded-xl uppercase tracking-widest text-xs">
                        Guardar Cambios
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        function showToast(msg, tipo = 'success') {
            ...
            inner.className = `rounded-2xl px-5 py-4 shadow-2xl font-bold text-sm text-white ${
                tipo === 'success' ? 'bg-[#39a900]' :
                tipo === 'error'   ? 'bg-red-700' :
                tipo === 'warn'    ? 'bg-yellow-600' : 'bg-blue-700'
            }`;
        ...
        function actualizarEstadoUI(estado) {
            ...
            badge.className = `flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider ` + (
                estado === 'disponible' ? 'bg-green-900/30 text-[#39a900] border border-[#39a900]/30' :
                estado === 'en_espera'  ? 'bg-yellow-900/30 text-yellow-400 border border-yellow-600/30' :
            ...
            if (estado === 'disponible') {
                ...
                btnAceptar.classList.add('pulse-green');
            } else {
                btnAceptar.disabled = true;
                btnAceptar.classList.add('opacity-40', 'cursor-not-allowed');
                btnAceptar.classList.remove('pulse-green');
            }
            ...
            try {
                ...
                if (data.historial.length === 0) {
                    ...
                    listaHist.innerHTML = data.historial.map(h => `
                        <div class="flex items-center justify-between glass rounded-xl px-5 py-3 border border-white/5">
                            <span class="text-[#39a900] font-black text-lg">${h.codigo}</span>
                            <p class="text-gray-400 text-xs font-semibold">
